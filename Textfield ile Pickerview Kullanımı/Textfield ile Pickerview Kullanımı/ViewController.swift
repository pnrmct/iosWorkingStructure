...
class ViewController: UIViewController, UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        textfieldUlke.inputView = pickerView //Tıknalıldığı zaman tetiklenmesi için
        ...
        let boslukButton = UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: nil, action: #selector(self.tamamTikla))
        
        let iptalButton = UIBarButtonItem(title: "İptal", style: .plain, target: self, action: #selector(self.iptalTikla))
        
        //Butonları toolbar'a tanımladık
    ...
    @objc func tamamTikla(){
        textfieldUlke.text = "" // içerisindeki veriyi sıfırlıyoruz.
