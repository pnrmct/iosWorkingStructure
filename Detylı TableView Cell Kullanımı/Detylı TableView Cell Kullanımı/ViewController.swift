...
class ViewController: UIViewController {
    ...
    var yemeklerListesi = [Yemekler]() // Yemekler sınıfından nesneler tutacak ve bunları aktif bir şekilde TableView üzerinde kullanabileceğiz.
...
extension ViewController:UITableViewDelegate,UITableViewDataSource,YemekTableViewCellProtocol {
    /*Hücre üzerindeki butona tıkladığım zaman çalıştıracak olan metodu tetiklememiz için butonun bulunduğu sınıfın protocolünü ekledik : YemekTableViewProtocol */
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return yemeklerListesi.count // dinamik olarak yemekler listesinde kaç veri varsa onu gösterecek.
    }// Arayüzdekileri cell' e (Hücre'ye) aktarıyoruz.
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let gelenYemek = yemeklerListesi[indexPath.row] // sırayla istediğimiz veriye erişebiliyoruz.
        
        let cell = tableView.dequeueReusableCell(withIdentifier: "yemekHucre", for: indexPath) as! YemekTableViewCell //Buradaki amaç görsel nesnelere erişmek.
